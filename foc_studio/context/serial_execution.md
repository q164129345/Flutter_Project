...
- `FocController` 仅展示快照并转发操作。有可见组件监听时最多每 50 ms 请求一次，
  状态未改变则不更新。普通业务快照只包含最新状态，不复制或传输历史列表；
  历史仍在后台按原容量持续采集。统计面板每秒读取一次；后台速率采样独立运行。
...
- 调参先编码全部六帧并检查总容量，再整组入队；编码失败或容量不足不会入队任何一帧。
  这是本地队列保证，不代表设备端事务或确认。
- 断开连接时取消旧的字节和消息订阅，丢弃未投递的旧数据，并清除半帧、待发送队列和业务状态；统计累计值保留到下次连接。
