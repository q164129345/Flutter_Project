...
/// 设置页专用：展示 MCU 通过串口协议返回的外部 Flash JEDEC ID。
class _ExternalFlashInfoPanel extends StatelessWidget {
  const _ExternalFlashInfoPanel({
  ...
  Widget build(BuildContext context) {
    return MotStylePanel(
      ...
      child: Wrap(
        ...
        children: [
          _ExternalFlashIdValue(label: '制造商ID', value: flashId?.manufacturerId),
          _ExternalFlashIdValue(label: '设备ID', value: flashId?.deviceId),
          FilledButton.icon(
...
class _ExternalFlashIdValue extends StatelessWidget {
  const _ExternalFlashIdValue({required this.label, required this.value});
