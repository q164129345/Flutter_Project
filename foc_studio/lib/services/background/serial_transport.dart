...
/// 后台串口传输层，只由常驻串口会话 isolate 创建和持有。
///
/// 枚举、打开、配置、读写和关闭本地串口都在后台调用；UI 只向代理发送请求。
/// SerialPortReader 另用读取 isolate 等待数据，再把字节送回本会话内的协议层。
class SerialTransport extends ChangeNotifier {
  SerialTransport({SerialStatisticsAccumulator? statistics})
    : statistics = statistics ?? SerialStatisticsAccumulator();
  ...
  void _setConnectionStatus(
    ...
    if (changed) {
      // 通知后台会话与连接状态转发器，后者将低频连接变化发给 UI 代理。
      notifyListeners();
    }
  }
  ...
  void _startReading() {
    ...
    _readerSubscription = reader.stream.listen(
      (data) {
        ...
        // Reader 已提供独立的字节块，解包器会写入自己的增量缓冲区。
        // 此处不重复复制，也不将原始字节转发到 UI isolate。
  ...
  int sendBytes(List<int> bytes) {
    ...
    // 不传 timeout，使用 libserialport 的非阻塞写。这个方法只在串口会话
    // isolate 上被调用，非阻塞写使接收和心跳可以继续
    // 执行；写缓冲区暂满时由协议发送队列稍后重试。
    //
